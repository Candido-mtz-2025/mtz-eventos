...
(function instalarTravaScrollDigitacaoPropostas() {
    if (window.__mtzTravaScrollDigitacaoPropostasInstalada) return;
    window.__mtzTravaScrollDigitacaoPropostasInstalada = true;

    let estadoScrollProposta = null;
    let bloqueioScrollAte = 0;
    let ultimoScrollManual = 0;
    let restaurandoScroll = false;

    function campoEditavelProposta(elemento) {
    ...
    function capturarScrollProposta(elemento = document.activeElement) {
        if (!campoEditavelProposta(elemento)) return;

        estadoScrollProposta = {
            x: window.scrollX || window.pageXOffset || 0,
            y: window.scrollY || window.pageYOffset || 0,
            elemento
        };
    ...
    function restaurarScrollProposta() {
        if (!estadoScrollProposta) return;
        if (Date.now() > bloqueioScrollAte) return;

        const ativo = document.activeElement;
        if (!campoEditavelProposta(ativo)) return;

        // Permite rolagem manual do usuário.
        if (Date.now() - ultimoScrollManual < 300) {
            estadoScrollProposta.x = window.scrollX || window.pageXOffset || 0;
            estadoScrollProposta.y = window.scrollY || window.pageYOffset || 0;
            return;
        }
        ...
        if (Math.abs(atualY - estadoScrollProposta.y) > 1 || Math.abs(atualX - estadoScrollProposta.x) > 1) {
            restaurandoScroll = true;
            window.scrollTo({
                top: estadoScrollProposta.y,
                left: estadoScrollProposta.x,
                behavior: 'auto'
            });
    ...
    function agendarRestauracaoScrollProposta() {
        requestAnimationFrame(restaurarScrollProposta);
        setTimeout(restaurarScrollProposta, 30);
        setTimeout(restaurarScrollProposta, 80);
        setTimeout(restaurarScrollProposta, 180);
        setTimeout(restaurarScrollProposta, 350);
        setTimeout(restaurarScrollProposta, 700);
        setTimeout(restaurarScrollProposta, 1200);
        setTimeout(restaurarScrollProposta, 2200);
    }
    ...
    document.addEventListener('focusin', (event) => {
        if (!campoEditavelProposta(event.target)) return;
        capturarScrollProposta(event.target);
    }, true);
    ...
    document.addEventListener('keydown', (event) => {
        ...
        const teclasRolagem = ['Tab', 'PageUp', 'PageDown', 'Home', 'End', 'ArrowUp', 'ArrowDown'];
        if (teclasRolagem.includes(event.key)) {
            ultimoScrollManual = Date.now();
            return;
        }
